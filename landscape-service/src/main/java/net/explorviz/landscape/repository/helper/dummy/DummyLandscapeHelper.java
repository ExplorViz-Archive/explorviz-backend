...
public final class DummyLandscapeHelper {
   ...
   */
  public static Landscape createLandscape() {
    return new Landscape(idGen.generateId(),
        new Timestamp(idGen.generateId(), java.lang.System.currentTimeMillis(), 0));
  }
  ...
  public static System createSystem(final String name, final Landscape parentLandscape) {
    final System newSystem = new System(idGen.generateId());
    newSystem.setName(name);
    newSystem.setParent(parentLandscape);

    parentLandscape.getSystems().add(newSystem);

    // create new system event
    parentLandscape.createNewEvent(idGen.generateId(),
        EEventType.NEWSYSTEM,
        "New system '" + newSystem.getName() + "' detected");
  ...
  public static NodeGroup createNodeGroup(final String name, final System system) {
    final NodeGroup newNodeGroup = new NodeGroup(idGen.generateId());
    newNodeGroup.setName(name);
    newNodeGroup.setParent(system);

    system.getNodeGroups().add(newNodeGroup);
  ...
  public static Node createNode(final String ipAddress, final NodeGroup parentNodeGroup,
    ...
    final Node newNode = new Node(idGen.generateId());
    newNode.setIpAddress(ipAddress);
    newNode.setParent(parentNodeGroup);

    parentNodeGroup.getNodes().add(newNode);

    // set random usage
    newNode.setCpuUtilization((double) getRandomNum(10, 100) / 100);
    newNode.setFreeRAM((long) getRandomNum(1, 4) * formatFactor);
    newNode.setUsedRAM((long) getRandomNum(1, 4) * formatFactor);

    // create a new node event
    landscape.createNewEvent(idGen.generateId(),
        EEventType.NEWNODE,
        "New node '" + newNode.getIpAddress() + "' in system '"
            + parentNodeGroup.getParent().getName() + "' detected");
  ...
  public static Application createApplication(final String name, final Node parentNode,
      final Landscape landscape, final EProgrammingLanguage programmingLanguage) {
    final Application newApplication = new Application(idGen.generateId());

    newApplication.setParent(parentNode);
    newApplication.setLastUsage(java.lang.System.currentTimeMillis());
    newApplication.setProgrammingLanguage(programmingLanguage);
    newApplication.setName(name);

    parentNode.getApplications().add(newApplication);

    // create a new application event
    landscape.createNewEvent(idGen.generateId(),
        EEventType.NEWNODE,
        "New node '" + newApplication.getName() + "' in system '" + parentNode.getParent().getName()
            + "' detected");
   ...
   * @param minNumOfRequests - minimum number of requests
   * @param maxNumOfRequests - maximum number of requests
   *
  ...
  public static ApplicationCommunication createApplicationCommunication(final Application source,
      final Application target, final Landscape landscape, final String technology,
      final int minNumOfRequests, final int maxNumOfRequests) {

    final int numberOfRequests = getRandomNum(minNumOfRequests, maxNumOfRequests);
    ...
    communication.setRequests(numberOfRequests);

    final float averageResponseTime = 0L + getRandomNum(100, 10000);
  ...
  public static Clazz createClazz(final String name, final Component component,
    ...
    final Clazz newClazz = new Clazz(idGen.generateId());
    newClazz.setName(name);
    newClazz.setFullQualifiedName(component.getFullQualifiedName() + "." + name);
    newClazz.setInstanceCount(instanceCount);
    newClazz.setParent(component);
    component.getClazzes().add(newClazz);
  ...
  public static void createClazzCommunication(final String traceId, final int tracePosition,
    ...
    // adds clazzCommunication to clazz and aggregatedClazzCommunication to application
