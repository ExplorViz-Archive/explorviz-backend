...
public class InsertionRepositoryPart {
	 ...
	 * TODODescr.
	 *
	 * @param inputIRecord             - Record that will be inserted into the
	 *                                 passed landscape
		...
		if (inputIRecord instanceof Trace) {
			...
			synchronized (landscape) {
				for (int i = 0; i < hostApplicationMetadataList.size(); i++) {
					...
					final System system = this.seekOrCreateSystem(landscape, hostApplicationRecord.getSystemname());

					final boolean isNewNode = this.nodeCache.get(
					...
					final Application application = this.seekOrCreateApplication(node, hostApplicationRecord,
							landscape);
	...
	protected Node seekOrCreateNode(final HostApplicationMetaDataRecord hostApplicationRecord,
		...
		if (node == null) {
			...
			this.addToEvents(landscape, "New node '" + hostApplicationRecord.getHostname() + "' in system '"
	 ...
	 */
	private void createCommuInApp(final Trace trace, final String currentHostname, final Application currentApplication,
			final Landscape landscape, final RemoteCallRepositoryPart remoteCallRepositoryPart,
		...
		for (int i = 0; i < eventsLength; i++) {
			...
			if (event instanceof AbstractBeforeOperationEventRecord) {
				...
				final Clazz currentClazz = this.seekOrCreateClazz(clazzName, currentApplication,
				...
				if (callerClazz != null) {
					...
					if (abstractBeforeEventRecord instanceof BeforeJDBCOperationEventRecord) {
						...
						final String parsedStatementType = operationSignature.contains(" ")
								? operationSignature.split(" ")[0]
								: operationSignature;
				...
				if (event instanceof AbstractAfterFailedEventRecord && callerClazz != null) {
					...
					this.addToErrors(landscape, "Exception thrown in application '" + currentApplication.getName()
				...
				if (i + 1 < eventsLength
					...
					firstReceiverClazz = this.seekOrCreateClazz(clazzName, currentApplication, abstractBeforeEventRecord
	...
	private void createOrUpdateCall(final Clazz caller, final Clazz callee, final Application application,
			final int requests, final double average, final double overallTraceDuration, final String traceId,
			final int orderIndex, final String operationName, final Landscape landscape) {

		landscape.getTimestamp().setTotalRequests(landscape.getTimestamp().getTotalRequests() + requests);
		...
		if (index < splittedName.length - 1) {
			...
			for (final Component component : list) {
				if (component.getName().equalsIgnoreCase(currentPart)) {
					return this.seekrOrCreateClazzHelper(fullQName, splittedName, application, component, index + 1);
				}
			}
