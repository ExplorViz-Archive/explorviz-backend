...
class UserCreation {
  ...
  @Test @DisplayName("Login with created user") public void loginWithCreatedUser()
    ...
    // Create user
    given().body(UserSerializationHelper.serialize(u)).header(authHeaderAdmin)
        .contentType(MEDIA_TYPE).when().post(BASE_URI + "users/");
  ...
  @Test @DisplayName("Create user without password") void createUserWithNoPassword() {
    User u = new User(null, "name", null, null);
