...
public class UserResource {
  ...
  @ApiResponse(responseCode = HttpStatus.OK, description = "User created",
               content = @Content(mediaType = MEDIA_TYPE,
                                  schema = @Schema(implementation = User.class)))
  @ApiResponse(responseCode = HttpStatus.BAD_REQUEST,
               description = "Invalid properties for the user or user with the given "
                   + "name already exists.")
  @RequestBody(
  ...
  @ApiResponse(responseCode = HttpStatus.OK, description = "Updated user",
               content = @Content(mediaType = MEDIA_TYPE,
                                  schema = @Schema(implementation = User.class)))
  ...
  public User updateUser(
    ...
    try {
      return this.userCrudService.updateEntity(id, updatedUser);
  ...
  @Parameters({
                  @Parameter(in = ParameterIn.QUERY, name = "page[size]",
                             description = "Controls the size, i.e., amount of entities, "
                                 + "of each page."),
                  ...
                  @Parameter(in = ParameterIn.QUERY, name = "filter[batchid]",
                             description =
                                 "Only return users that were created by the batch request "
                                     + "with the specified id.")})
  @ApiResponse(responseCode = HttpStatus.OK, description = "List of users matching the request",
               content = @Content(
  ...
  @ApiResponse(responseCode = HttpStatus.OK, description = "The requested user",
               content = @Content(mediaType = MEDIA_TYPE,
                                  schema = @Schema(implementation = User.class)))
  ...
  public User userById(
    ...
    return this.userCrudService.getEntityById(id).orElseThrow(NotFoundException::new);
  }
  ...
  public Response removeUser(@Parameter(
    ...
    return Response.status(NO_CONTENT_204).build();
  }
