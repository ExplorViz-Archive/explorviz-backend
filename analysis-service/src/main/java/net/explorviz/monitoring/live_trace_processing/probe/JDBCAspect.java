...
public class JDBCAspect {
	...
	/* Configuration */
	private static final String NOT_WITHIN = "!within(explorviz..*) && !within(com.lmax..*) && !within(org.hyperic.sigar..*)";
	...
	public final Object operation(final Object thisObject, final ProceedingJoinPoint thisJoinPoint)
		...
		if (BeforeJDBCOperationEventRecord.COMPRESSED_BYTE_LENGTH_WITH_CLAZZ_ID > buffer
				.remaining()) {
		...
		buffer.putInt(MonitoringStringRegistry.getIdForString(getInterface(thisJoinPoint)));

		processMonitoringBeforeEvent(thisJoinPoint, buffer);
		...
		try {
			...
			if (AfterFailedJDBCOperationEventRecord.COMPRESSED_BYTE_LENGTH_WITH_CLAZZ_ID > buffer
					.remaining()) {
				// updateLastSendingTimeToCurrent();
				MonitoringController.sendOutBuffer(buffer);
			}
		...
		processMonitoringAfterEvent(thisJoinPoint, retval, buffer);
	...
	private void processMonitoringBeforeEvent(final ProceedingJoinPoint currentJoinPoint,
		...
		String stringClassArgs = convertJoinpointArgsToString(classArgs);
	...
	private String extractJoinPointReturnValue(final String className, final Object returnValue) {
		...
		if ((className != null) && (returnValue != null)) {
			final String returnType = getJoinPointReturnType(className).toUpperCase();
