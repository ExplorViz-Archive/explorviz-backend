...
/**
 * This Probe monitors incoming and outgoing JAX-WS messages. Therefore it has
 * to be added to the JAX-WS handler chain. The handler chain is added by the
 * JAXWSProbeAdder aspect. It can also be added manually with an
...
public class JAXWSProbe implements SOAPHandler<SOAPMessageContext> {
	...
	public boolean handleMessage(final SOAPMessageContext context) {
		...
		if (probeController.isMonitoringEnabled()) {
			...
			final Boolean isOutbound = (Boolean) context
			...
			try {
				...
				if (outgoing) {
					...
					if (isClientSide) {
						...
						final SOAPHeaderElement traceIDElement = soapHeader
								.addHeaderElement(qnameTraceId);
						traceIDElement.setActor(SOAPConstants.URI_SOAP_ACTOR_NEXT);
						traceIDElement.addTextNode(ownTraceId.toString());

						final SOAPHeaderElement soapHeaderElement2 = soapHeader
								.addHeaderElement(qnameOrderId);
						soapHeaderElement2.setActor(SOAPConstants.URI_SOAP_ACTOR_NEXT);
						soapHeaderElement2.addTextNode(ownOrderId.toString());

						soapMsg.saveChanges();

						DistributedMonitoringRecordWriter.writeBeforeSentRecord(ownTraceId,
								ownOrderId, "JAX-WS");
					} else {
						final Long ownTraceId = trace.getTraceId();
						final Integer ownOrderId = trace.getNextOrderId();

						DistributedMonitoringRecordWriter.writeAfterReceivedRecord(ownTraceId,
								ownOrderId);
					...
					if (soapHeader != null) {
						...
						if (!isClientSide) {
							...
							final Iterator<SOAPHeaderElement> headerIterator = soapHeader
							...
							if (foundTraceInformation) {
								DistributedMonitoringRecordWriter.writeBeforeReceivedRecord(
										ownTraceId, ownOrderId, remoteTraceId, remoteOrderId);
							}
						} else {
							final Long ownTraceId = trace.getTraceId();
							final Integer ownOrderId = trace.getNextOrderId();

							DistributedMonitoringRecordWriter.writeAfterSentRecord(ownTraceId,
									ownOrderId);
