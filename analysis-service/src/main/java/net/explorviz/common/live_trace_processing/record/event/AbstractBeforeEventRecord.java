...
public abstract class AbstractBeforeEventRecord extends AbstractEventRecord {
  ...
  protected static final int BYTE_LENGTH =
      AbstractEventRecord.BYTE_LENGTH + RuntimeStatisticInformation.BYTE_LENGTH;
  ...
  public AbstractBeforeEventRecord(final ByteBuffer buffer,
    ...
    this.runtimeStatisticInformationList = new ArrayList<>(runtimeSize);

    for (int i = 0; i < runtimeSize; i++) {
      this.runtimeStatisticInformationList
          .add(RuntimeStatisticInformation.createFromByteBuffer(buffer, stringRegistry));
    }
  }

  public List<RuntimeStatisticInformation> getRuntimeStatisticInformationList() {
    return this.runtimeStatisticInformationList;
  }
  ...
  public void putIntoByteBuffer(final ByteBuffer buffer, final StringRegistrySender stringRegistry,
    ...
    buffer.putInt(this.getRuntimeStatisticInformationList().size());

    for (final RuntimeStatisticInformation runtime : this.getRuntimeStatisticInformationList()) {
  ...
  public int getRecordSizeInBytes() {
    return 4
        + RuntimeStatisticInformation.BYTE_LENGTH * this.getRuntimeStatisticInformationList().size()
  ...
  public String toString() {
    return AbstractBeforeEventRecord.class.getSimpleName() + " - " + super.toString()
        + ", getRuntime()=" + this.getRuntimeStatisticInformationList();
  }
}
