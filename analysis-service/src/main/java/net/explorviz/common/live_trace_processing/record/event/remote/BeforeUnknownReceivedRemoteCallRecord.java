...
public class BeforeUnknownReceivedRemoteCallRecord extends
		AbstractBeforeEventRecord {
	public static final byte CLAZZ_ID = 16;
	public static final byte CLAZZ_ID_FROM_WORKER = CLAZZ_ID + 64;

	public static final int COMPRESSED_BYTE_LENGTH = AbstractBeforeEventRecord.COMPRESSED_BYTE_LENGTH + 4 + 4;
	...
	public BeforeUnknownReceivedRemoteCallRecord(final String sender,
			final String destination, final long traceId, final int orderIndex,
	...
	public BeforeUnknownReceivedRemoteCallRecord(final ByteBuffer buffer,
		...
		sender = stringRegistry.getStringFromId(buffer.getInt());
		destination = stringRegistry.getStringFromId(buffer.getInt());
	}
	...
	public void putIntoByteBuffer(final ByteBuffer buffer,
		...
		buffer.putInt(stringRegistry.getIdForString(sender));
		buffer.putInt(stringRegistry.getIdForString(destination));
	}

	@Override
	public int compareTo(final IRecord o) {
		if (o instanceof BeforeUnknownReceivedRemoteCallRecord) {
			final BeforeUnknownReceivedRemoteCallRecord record2 = (BeforeUnknownReceivedRemoteCallRecord) o;
			final int cmpSender = getSender().compareTo(record2.getSender());
			if (cmpSender != 0) {
				return cmpSender;
			}

			final int cmpDestination = getDestination().compareTo(
					record2.getDestination());
			if (cmpDestination != 0) {
	...
	public String toString() {
		return BeforeSentRemoteCallRecord.class.getSimpleName() + " - "
				+ super.toString() + ", getSender()=" + getSender()
