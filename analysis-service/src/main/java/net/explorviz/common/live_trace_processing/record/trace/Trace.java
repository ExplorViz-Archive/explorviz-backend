...
public final class Trace implements IRecord {
  ...
  public Trace(final List<AbstractEventRecord> traceEvents, final boolean valid,
      final boolean containsRemoteRecord) {
  ...
  public boolean containsRemoteRecord() {
    return this.containsRemoteRecord;
  }
  ...
  public int compareTo(final IRecord o) {
    final List<AbstractEventRecord> recordsT1 = this.getTraceEvents();
    if (o instanceof Trace) {
      final Trace otherTrace = (Trace) o;
      final List<AbstractEventRecord> recordsT2 = otherTrace.getTraceEvents();
      ...
      for (int i = 0; i < recordsT1.size(); i++) {
        final int cmpRecord = recordsT1.get(i).compareTo(recordsT2.get(i));
        if (cmpRecord != 0) {
          return cmpRecord;
        }
      }
  ...
  public String toString() {
    ...
    sb.append("\n\tTrace: ");
    for (final AbstractEventRecord traceEvent : this.traceEvents) {
      sb.append("\n\t");
