...
public class BeforeReceivedRemoteCallRecord extends AbstractBeforeEventRecord {
  ...
  public static final int COMPRESSED_BYTE_LENGTH =
      AbstractBeforeEventRecord.COMPRESSED_BYTE_LENGTH + 8 + 4;
  ...
  public BeforeReceivedRemoteCallRecord(final long callerTraceId, final int callerOrderIndex,
  ...
  public void putIntoByteBuffer(final ByteBuffer buffer, final StringRegistrySender stringRegistry,
    ...
    buffer.putLong(this.callerTraceId);
    buffer.putInt(this.callerOrderIndex);
  }
  ...
  public int compareTo(final IRecord o) {
    if (o instanceof BeforeReceivedRemoteCallRecord) {
      ...
      final int cmpOrderIndex = this.getCallerOrderIndex() - record2.getCallerOrderIndex();
      if (cmpOrderIndex != 0) {
        return cmpOrderIndex;
      }

      final long cmpCallerTraceId = this.getCallerTraceId() - record2.getCallerTraceId();
  ...
  public String toString() {
    return BeforeReceivedRemoteCallRecord.class.getSimpleName() + " - " + super.toString()
        + ", getCallerTraceId()=" + this.getCallerTraceId() + ", getCallerOrderIndex()="
