...
public final class CountingThroughputFilter implements
		IPeriodicTimeSignalReceiver {

	private final AtomicInteger currentCountForCurrentInterval = new AtomicInteger(
			0);
	...
	public CountingThroughputFilter(final String beforeOutMessage,
			final boolean enabled, final int outputIntervalInMillisec) {
	...
	public final void inputRecord(final IRecord record) {
		if (enabled && !(record instanceof SystemMonitoringRecord)
	...
	public final void inputObjectsCount(final int increment) {
		currentCountForCurrentInterval.addAndGet(increment);
	}
	...
	public void periodicTimeSignal(final long timestamp) {
		...
		System.out.println(timestamp + " " + beforeOutMessage + ": " + rest);
	}
