...
public class RemoteConfigurationServlet implements Runnable {
	...
	private final static Set<String> connectedChildren = new HashSet<String>(); // TODO
																				// concurrent...
					...
					if (socketChannel.read(buffer) != -1) {
						...
						while (buffer.remaining() > 0) {
							...
							if (action == 1 || action == 2) {
								final String application = readString(buffer);
								final String pattern = readString(buffer);
	...
	public static void removePattern(final String application,
