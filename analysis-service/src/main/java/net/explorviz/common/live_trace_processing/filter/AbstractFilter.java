...
public abstract class AbstractFilter extends Thread implements
		IPipeReceiver<IRecord> {
	protected final CountingThroughputFilter counter;

	private final Queue<IRecord> receiverQueue;

	public AbstractFilter(final Queue<IRecord> receiverQueue,
			final String counterString, final int outputIntervalInMillisec) {
		this.receiverQueue = receiverQueue;

		counter = new CountingThroughputFilter(counterString, true,
				outputIntervalInMillisec);
	}
	...
	protected final void deliver(final IRecord record) {
		counter.inputRecord(record);

		while (!receiverQueue.offer(record)) {
			try {
				Thread.sleep(1);
