...
public class LandscapeResource {
    ...
    public LandscapeResource(final LandscapeRepository<String> landscapeStringRepo,
        LandscapeRepository<Landscape> landscapeRepository,
    ...
    @ApiResponse(responseCode = "404",
        description = "No landscape with such id.")
    public String getLandscapeById(
        @Parameter(description = "Id of the landscape",
            required = true)
        @PathParam("id")
        final String id) {
    ...
    public String getLandscape(
        @Parameter(description = "The timestamp to filter by.",
            required = true)
        ...
        // Check existence in landscapeRepo and replayRepo or throw Exception
        // this can be done better since Java 9
        return Stream.of(this.landscapeStringRepo.getByTimestamp(timestamp),
            this.replayStringRepo.getByTimestamp(timestamp)).filter(Optional::isPresent)
            .map(Optional::get).findFirst().orElseThrow(() -> new NotFoundException(
                "Landscape with timestamp " + timestamp + " not found."));
    }


    /**
     * Access systems of that landscape
     * @return Instance of {@link SystemSubResource} to provide the systems associated to this landscape
     */
    @Path("{id}/systems")
    public SystemSubResource getSystems(
        @Parameter(description = "Id of the landscape",
            required = true)
        @PathParam("id")
        final String id) {

        java.lang.System.out.println("landscape id:" + id);
        Landscape l = landscapeRepository.getById(id).orElseThrow(NotFoundException::new);



        return new SystemSubResource(l);
    }
    ...
    @ApiResponse(responseCode = "404",
        description = "No landscape with the given timestamp.")
    public String downloadLandscape(
        @Parameter(description = "The timestamp to filter by.",
            required = true)
        @QueryParam("timestamp")
        final long timestamp) {
        ...
        if (found == null) {
            ...
            try {
                ...
                this.replayStringRepo
                    .save(parsedLandscape.getTimestamp().getTimestamp(), parsedLandscape,
